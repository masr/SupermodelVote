...
//print 'getspace';
  			$_SN[$space['uid']] = $space['name']; //njivy 替换成真实名 郭壮 3.23
...
//njivy 陈进 更新社团积分
function updateclubstatus( $clubid,$creditmode, $optype) {
	global $_SGLOBAL;
	...
	//更新状态
	$_SGLOBAL['db']->query("UPDATE ".tname('club')." SET  $creditsql WHERE clubid='$clubid'"); 
}  
...
function getstar($credit) {
	...
	if($_SCONFIG['starcredit'] > 1) {
		...
		if($_SCONFIG['starlevelnum'] < 2) {
			...
			for($i = 0; $i < $starnum; $i++) {
				$starimg .= '<img src="image/star_level10.gif" align="absmiddle" width="25" height="25"/>';
			}
		} else {
			//计算等级(10个)
			for($i = 10; $i > 0; $i--) {
				...
				if($numlevel) {
					for($j = 0; $j < $numlevel; $j++) {
						$starimg .= '<img src="image/star_level'.$i.'.gif" align="absmiddle" width="25" height="25"/>';
					}
					break;
				}
			}
	...
	if(empty($starimg)) $starimg = '<img src="image/credit.gif" alt="'.$credit.'" align="absmiddle" alt="'.$credit.'" title="'.$credit.'" width="25" height="25"/>';
	return $starimg;
}
...
//是smulti与multi的结合，样式为multi  pjj
function smulti_multi($start, $perpage, $count, $url, $ajaxdiv='',$default_inajax=0,$num=0) {
	global $_SGLOBAL;
	
	$mpurl=$url;
	$curpage=$start/$perpage+1;
	$page = 5;
	$multipage = '';
	$realpages = 1;
	if($num > $perpage) 
	{
		...
		if($_SGLOBAL['inajax'] || $default_inajax) 
		{
			 $mpurl="javascript:;\" onclick=\"ajaxget('$mpurl";
		}
		 
		$multipage = ($curpage - $offset > 1 && $pages > $page ? '<a href="'.$mpurl.'&page=1\',\''.$ajaxdiv.'\')" class="first">1 ...</a>' : '').
			($curpage > 1 ? '<a href="'.$mpurl.'&page='.($curpage - 1).'\',\''.$ajaxdiv.'\')" class="prev">&lsaquo;&lsaquo;</a>' : '');
		for($i = $from; $i <= $to; $i++) {
			$multipage .= $i == $curpage ? '<strong>'.$i.'</strong>' :
				'<a href="'.$mpurl.'&page='.$i.'\',\''.$ajaxdiv.'\')">'.$i.'</a>';
		}
		$multipage .= ($curpage < $pages ? '<a href="'.$mpurl.'&page='.($curpage + 1).'\',\''.$ajaxdiv.'\')" class="next">&rsaquo;&rsaquo;</a>' : '').
			($to < $pages ? '<a href="'.$mpurl.'&page='.$pages.'\',\''.$ajaxdiv.'\')" class="last">... '.$realpages.'</a>' : '');
		$multipage = $multipage ? ('<em>&nbsp;'.$num.'&nbsp;</em>'.$multipage):'';
	}
...
function getclub($id) {
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('club')." WHERE clubid='$id'");
	if(!$club = $_SGLOBAL['db']->fetch_array($query)) {
		showmessage('designated_election_it_does_not_exist');
	}
	//空社团
	if($club['membernum']<1 && ($club['joinperm'] || $club['viewperm'])) {
		$club['joinperm'] = $club['viewperm'] = 0;
		updatetable('club', array('joinperm'=>0, 'viewperm'=>0), array('clubid'=>$id));
	}
	
	//处理
	include_once(S_ROOT.'./data/data_profield.php');
	$club['field'] = $_SGLOBAL['profield'][$club['fieldid']];
	$club['title'] = $club['field']['title'];
	if(empty($club['pic'])) {
		$club['pic'] = 'image/nologo.jpg';
	}

	//成员级别
	$club['ismember'] = 0;
	$club['grade'] = -9;//-9 非成员 -2 申请 -1 禁言 0 普通 1 明星 8 副群主 9 群主
	$query = $_SGLOBAL['db']->query("SELECT grade FROM ".tname('tagspace')." WHERE clubid='$id' AND uid='$_SGLOBAL[supe_uid]' LIMIT 1");
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
		$club['grade'] = $value['grade'];
		$club['ismember'] = 1;
	}
	if($club['grade'] < 9 && checkperm('manageclub')) {
		$club['grade'] = 9;
	}
	$club['allowpost'] = $club['grade']>=0?1:0;
	$club['allowview'] = ($club['viewperm'] && $club['grade'] < -1)?0:1;
	
	$club['allowinvite'] = $club['grade']>=0?1:0;
	if($club['joinperm'] && $club['grade'] < 8) {
		$club['allowinvite'] = 0;
	}
...
//整理clubfeed
function mkclubfeed($feed, $actors=array()) {
	global $_SGLOBAL, $_SN, $_SCONFIG;
	...
	$searchs[] = '{actor}';
	$replaces[] = empty($actors)?"<a href=\"space.php?clubid=$feed[clubid]\">".$_SN[$feed['clubid']]."</a>":implode(lang('dot'), $actors);
	$searchs[] = '{app}';
	if(empty($_SGLOBAL['app'][$feed['appid']])) {
		$replaces[] = '';
	} else {
	...
	$searchs[] = '{actor}';
	$replaces[] = "<a href=\"space.php?clubid=$feed[clubid]\">$feed[clubname]</a>";

	$feed['body_template'] = mkclubtarget(str_replace($searchs, $replaces, $feed['body_template']));

	$feed['body_general'] = mkclubtarget($feed['body_general']);
...
//njivy chenjin 3-15整理selffeed的链接
function mkselftarget($html) {
	global $_SCONFIG;
...
function mkselffeed($feed, $actors=array()) {
	global $_SGLOBAL, $_SN, $_SCONFIG;
 ...
 //获取友情社团状态
function getclubsfriendstatus($clubid, $fclubid) {
	global $_SGLOBAL;

	$query = $_SGLOBAL['db']->query("SELECT status FROM ".tname('fclub')." WHERE clubid='$clubid' AND fclubid='$fclubid' LIMIT 1");
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
		return $value['status'];
	} else {
		return -1;
 ...
 function getclubinfodll($clubid) {
		global $_SGLOBAL;
		$club_user=array();		
		$query = $_SGLOBAL['db']->query("SELECT ts.* FROM " .tname('club')." as ts where ts.clubid =$clubid ");
...
function getclubinfo($clubid) {
		global $_SGLOBAL;
		$club_user=array();		
		$query = $_SGLOBAL['db']->query("SELECT ts.uid,ts.username FROM " .tname('tagspace')." as ts where ts.clubid =$clubid ");
